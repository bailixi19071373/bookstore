...
public class BookDao {
	 ...
	 */
	public PageInfo<Book> selectByCondition(Book condition,String authorname,PageInfo<Book> page) throws SQLException, ClassNotFoundException {
		Connection conn = null;
		try {
			conn = DBInfo.getConnnection();
			String sqlTotalRow = "select count(distinct b.isbn) from tbl_books b "
					+ "join tbl_bookauthor ba on b.ISBN = ba.isbn " + 
					"  join tbl_author au on ba.authorid = au.authorid "
			...
			while (rs.next()) {
				...
				if(rs2.getInt(1) > 0) {
					...
					while(rs3.next()) {
						authors.add(new Author(rs3.getString("authorid"), 
								rs3.getString("name")));
					}
	...
	public Book selectOneByISBN(String isbn) throws ClassNotFoundException, SQLException {
		Connection conn = null;
		try {
			conn = DBInfo.getConnnection();
	...
	public Book selectOneAndPublisherAuthorByISBN(String isbn) throws ClassNotFoundException, SQLException {
		Connection conn = null;
		try {
			conn = DBInfo.getConnnection();
			String sql = "select * from tbl_books b join tbl_publisher p " + "on b.pubid = p.pubid where isbn=?";
			PreparedStatement pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, isbn);
			ResultSet rs = pstmt.executeQuery();
			if (rs.next()) {
	...
	public void updateOne(Book book) throws ClassNotFoundException, SQLException {
		...
		try {
			conn = DBInfo.getConnnection();
			conn.setAutoCommit(false);
			String sql = "update tbl_books set title=?," + "pubdate=?,pubid=?,cost=?,retail=?,category=? where isbn=?";
			PreparedStatement pstmt = conn.prepareStatement(sql);
